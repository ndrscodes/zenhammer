...
void FuzzingParameterSet::print_dynamic_parameters2(int wait_until_hammering_us) {
...
void FuzzingParameterSet::randomize_parameters(bool print) {
  ...
  if (fixed_acts_per_trefi > 0) {
    ...
    num_activations_per_tREFI = Range<int>(40, 80).get_random_number(gen);
  }
  ...
  amplitude = Range<int>(1, num_activations_per_tREFI);
  ...
  // [derivable from aggressor_to_addr (DRAMAddr) in PatternAddressMapper]
  auto max_start_row = (int)DRAMConfig::get().rows() / 4;
  start_row = Range<int>(0, max_start_row);
...
//  auto strategy = get_valid_strategy_pair();
  ...
  max_row_no = (int)DRAMConfig::get().rows();
